...
public sealed class PairwiseColorExclusionAttribute(Color color) : MatrixExclusionAttribute(color);
...
public class PairwiseDataSourceAttributeTests
{
    ...
    [Test]
    [PairwiseDataSource]
    [PairwiseColorExclusion(Color.Green)]
    public async Task ExplicitEnumMatrix_RespectsDerivedExclusion(
        [Matrix(Color.Red, Color.Green, Color.Blue)] Color color)
    {
        await Assert.That(color).IsNotEqualTo(Color.Green);
        await Assert.That(color is Color.Red or Color.Blue).IsTrue();
    }

    [Test]
    [PairwiseDataSource]
    [MatrixExclusion(Color.Red, Color.Green)]
    public async Task MixedAutoGeneratedAndExplicitEnums_RespectExclusion(
        Color autoGenerated,
        [Matrix(Color.Green, Color.Blue)] Color explicitMatrix)
    {
        await Assert.That((autoGenerated, explicitMatrix)).IsNotEqualTo((Color.Red, Color.Green));
        await Assert.That(explicitMatrix is Color.Green or Color.Blue).IsTrue();
    }

    [Test]
    [PairwiseDataSource]
    public async Task MatrixMethod_WithEnumParameter_UsesOnlyMethodValues(
        [Matrix(true, false)] bool enabled,
        [MatrixMethod<PairwiseDataSourceAttributeTests>(nameof(GetSpecificColors))] Color color)
    {
        await Assert.That(enabled || !enabled).IsTrue();
        await Assert.That(color is Color.Red or Color.Blue).IsTrue();
    }

    [Test]
    [PairwiseDataSource]
    public async Task MatrixRange_WithExcluding_FiltersValues(
        [MatrixRange<int>(1, 5, Excluding = [2, 4])] int value,
        [Matrix("left", "right")] string side)
    {
        await Assert.That(value is 1 or 3 or 5).IsTrue();
        await Assert.That(side is "left" or "right").IsTrue();
    }

    [Test]
    [PairwiseDataSource]
    public async Task MatrixInstanceMethod_UsesInstanceValues(
        [MatrixInstanceMethod<PairwiseDataSourceAttributeTests>(nameof(GetInstanceValues))] string value,
        [Matrix("A", "B")] string suffix)
    {
        await Assert.That(value).IsEqualTo("instance-value");
        await Assert.That(["A", "B"]).Contains(suffix);
    }

    public static IEnumerable<Color> GetSpecificColors()
    {
        yield return Color.Red;
        yield return Color.Blue;
    }

    public IEnumerable<string> GetInstanceValues()
    {
        yield return $"{_prefix}-value";
    }
}
